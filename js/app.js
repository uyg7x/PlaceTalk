...
/* ── GEOLOCATION ── */
const GEO_ACCEPT_ACCURACY = 150;
const GEO_GOOD_ENOUGH_ACCURACY = 800;
const GEO_COARSE_WARNING_ACCURACY = 2500;
const GEO_HARD_FAIL_ACCURACY = 12000;
...
function getLocation(){
  ...
  getBestBrowserLocation()
    .then(async pos=>{
      ...
      setLoad(acc <= GEO_GOOD_ENOUGH_ACCURACY ? 'Precise location found! Searching nearby…' : 'Approximate location found. Searching nearby…',48);
      reverseGeocode(userLat,userLng,acc);

      if(acc > GEO_COARSE_WARNING_ACCURACY){
        toast('Your device gave an approximate location. Results can be off. Turn on precise location in browser/Windows or use mobile for better accuracy.','info');
      }
...
function getBestBrowserLocation(){
  return new Promise((resolve,reject)=>{
    ...
    let finish = (kind, value) => {
      if(settled) return;
      settled = true;
      if(watchId !== null){
        try{ navigator.geolocation.clearWatch(watchId); }catch(e){}
      }
      kind === 'resolve' ? resolve(value) : reject(value);
    };

    const rank = pos => Number.isFinite(pos?.coords?.accuracy) ? pos.coords.accuracy : Number.POSITIVE_INFINITY;
    const consider = pos => {
      if(!pos?.coords) return;
      if(!best || rank(pos) < rank(best)) best = pos;

      const acc = rank(best);
      if(acc <= GEO_ACCEPT_ACCURACY){
        finish('resolve', best);
      }
    };
    ...
    const overallTimer = setTimeout(()=>{
      if(best){
        if(rank(best) > GEO_HARD_FAIL_ACCURACY){
          finish('reject', { code:'COARSE_LOCATION', accuracy: rank(best) });
          return;
        }
        finish('resolve', best);
        return;
    ...
    finish = (kind, value) => {
      ...
      wrappedFinish(kind, value);
    };

    const geoOptions = { enableHighAccuracy:true, timeout:7000, maximumAge:0 };
...
function getGeoErrorMessage(err){
  if(err?.code === 'COARSE_LOCATION'){
    return 'Your device returned a very rough location, so PlaceTalk did not trust it. Enable precise location in Windows/browser settings or open the site on mobile.';
  }
  return err?.code===1 ? 'Location access denied. Please allow in browser settings.'
       : err?.code===2 ? 'Location unavailable. Turn on Wi‑Fi/GPS and try again.'
       : 'Location timed out. Try again near a window, with Wi‑Fi on, or on mobile.';
}

function formatAccuracyText(acc){
  if(!Number.isFinite(acc) || acc <= 0) return '';
  if(acc < 1000) return ` · accuracy ~${Math.round(acc)} m`;
  return ` · accuracy ~${(acc/1000).toFixed(1)} km`;
}
...
function reverseGeocode(lat,lng,accuracy){
  fetch(`https://nominatim.openstreetmap.org/reverse?format=json&lat=${lat}&lon=${lng}&addressdetails=1`)
    .then(r=>r.json()).then(d=>{
      const a=d.address||{};
      const parts=[a.suburb||a.neighbourhood||a.village,a.city||a.town||a.county||a.state_district].filter(Boolean);
      const txt=parts.slice(0,2).join(', ')||'Your Location';
      const el=g('locText'); if(el)el.textContent='📍 '+txt + formatAccuracyText(accuracy);
    }).catch(()=>{
      const el=g('locText');
      if(el)el.textContent=`📍 ${lat.toFixed(4)}, ${lng.toFixed(4)}${formatAccuracyText(accuracy)}`;
    });
}
